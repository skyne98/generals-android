...
public class SDLSurface extends SurfaceView implements SurfaceHolder.Callback,
    ...
    public void surfaceChanged(SurfaceHolder holder,
        ...
        // SurfaceFlinger otherwise treats SDL's SurfaceView as 60 Hz even when
        // the Activity selected the phone's 120 Hz display mode.
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            float highestRefreshRate = mDisplay.getRefreshRate();
            for (Display.Mode mode : mDisplay.getSupportedModes()) {
                highestRefreshRate = Math.max(highestRefreshRate, mode.getRefreshRate());
            }
            holder.getSurface().setFrameRate(highestRefreshRate,
